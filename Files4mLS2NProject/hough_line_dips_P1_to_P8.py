...
if __name__ == '__main__':
    ...
    pulley_num_list = [1]
    df        = pd.DataFrame()
    for pulley_num in pulley_num_list:
        
        filename = 'Set02/P'+str(pulley_num)+'/D2_Set2_P'+str(pulley_num)+'_T51.mp4'
        ...
        try:
            ...
            while(vidcap.isOpened()):
                ...
                # Prepare crop area
                if pulley_num < 7:
                    width_percent, height_percent = 1, 0.4
                    h, w, c = image.shape
                ...
                # 2. to binary image
                if pulley_num < 7:
                    ret, thresh1 = cv2.threshold(gray, 200, 300, cv2.THRESH_BINARY)
                ...
                numlines = 4
                angles = []
                if pulley_num == 1 or pulley_num == 7:
                    lines = cv2.HoughLines(edges, rho=1, theta=np.pi/180, threshold=150)
                elif pulley_num>1 and pulley_num<7:
                ...
                # calc the average
                #sum(hor_lines_rho) / len(hor_lines_rho)
                if pulley_num == 1:
                    hor_rho_ref = 930
                    hor_theta_ref = 88/180*np.pi#sum(hor_lines_theta) / len(hor_lines_theta)
                elif pulley_num == 2:
                    hor_rho_ref = 930
                    hor_theta_ref = 90/180*np.pi#
                elif pulley_num == 3:
                    hor_rho_ref = 950
                    hor_theta_ref = 87.5/180*np.pi
                elif pulley_num == 4:
                    hor_rho_ref = 930
                    hor_theta_ref = 90/180*np.pi
                elif pulley_num == 5:
                    ...
                    hor_rho_ref = 960
                    hor_theta_ref = 87/180*np.pi
                elif pulley_num == 8:
                    hor_rho_ref = 980
                    hor_theta_ref = 91/180*np.pi
                else:
                    print('No pulley found')
            ...
            pulley_num_str = 'P'+str(pulley_num)            
            if pulley_num_str in ['P1','P2','P3','P4','P5','P6','P7','P8']:
                df_pulley        = pd.DataFrame({pulley_num_str:list(angDArray[1:,0])})
                df = pd.concat([df,df_pulley],axis = 1)
            
            # When everything done, release the video capture object
            vidcap.release()
            # Closes all the frames
            cv2.destroyAllWindows()
            print("\nQuit and proceed to next pulley")
            ...
            plt.plot(angDArray[1:])
            
            # df.columns = ('P1','P2','P3','P4','P5','P6','P7','P8')
            filename_save = filename.split('/')[0]+'/'+filename.split('/')[2].split('.')[0]+'.csv'
            filename_save = filename_save.split('_')[0]+'_'+\
                filename_save.split('_')[1]+'_PA_'+filename_save.split('_')[3]
